 ...
 * Changelog:
 * v1.3 (2021-07-12)
    + Removed pointless "if statement"
 * v1.2 (2021-07-12)
    + Bug Fix
 * v1.1 (2021-07-09)
 	+ Initial Release
...
for i = 0, reaper.CountSelectedMediaItems(0)-1 do
    ...
    for j = track_index, reaper.CountTracks(0)-1 do
        Msg("index is " .. j)
        -- break loop if we are at the last track, or if track depth after index j is equal to track depth at index j
        if j == reaper.CountTracks(0)-1 or reaper.GetTrackDepth(reaper.GetTrack(0, j+1)) <= reaper.GetTrackDepth(track) then 
            break end
        index = index + 1
    end
